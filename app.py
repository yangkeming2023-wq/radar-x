...
with col1: gender = st.selectbox("性别", ["乾造", "坤造"])
...
if st.button("执行全景推演"):
    if birth_date and birth_time:
        ...
        with st.spinner(f"正在进行算法级精密推演..."):
            try:
                response = client.chat.completions.create(
                    model="deepseek-reasoner",
                    messages=[
                        {"role": "system", "content": f"""你是一个顶尖的八字大师，和倪海厦大师一样有实力，学识渊博，精通《渊海子平》《三命通会》《滴天髓》等古籍和盲派命理的推算方法。
                        要求：
                        ...
                        6. 实事求是根据南北道教以及上述逻辑直白的阐述真实情况。
                        严禁谄媚，严禁缝合数据。"""},
                        {"role": "user", "content": f"""
                        命主信息：{gender}，{age}岁。
                        排盘数据：{chart_table}
                        推演指令：请严格按照以下六大模块进行分析：
                        1.【出厂设置】：结合{gender}与{age}岁特征，分析天赋内核与性格逻辑。
                        2.【人生三大真相】：深刻洞察该命局必然面对的三大核心规律。
